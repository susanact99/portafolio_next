      ...
      <CardContent className="p-6">
        <div className="flex flex-col md:flex-row gap-6">
          ...
          <div className="flex flex-col items-center md:items-start gap-3 md:w-32 flex-shrink-0">
            <div
              className={`w-12 h-12 rounded-full bg-gradient-to-r ${getTypeColor()} flex items-center justify-center text-white neon-glow`}
            >
              {getIcon()}
            </div>
            ...
            {status === "in-progress" && (
              <Badge variant="secondary" className="glass-effect text-xs flex items-center gap-1">
        ...
        {status === "in-progress" && (
          <div className="mt-4 pt-4 border-t border-border/50">
            <div className="flex justify-between items-center mb-2">
              <span className="text-sm text-muted-foreground">Progreso estimado</span>
              <span className="text-sm font-medium">75%</span>
            </div>
            <div className="w-full bg-muted rounded-full h-2">
              <div className="bg-gradient-to-r from-primary to-blue-500 h-2 rounded-full w-3/4 transition-all duration-300"></div>
            </div>
          </div>
        )}
      </CardContent>
